...
if __name__=="__main__":
    ...
    # Adding arguments
    parser.add_argument("--save_path",default="phi4_final_results.json", help = "path to Output file")
                ...
                try:
                    ...
                    if not video_flag:
                        if len(sample_path) == 1:
                            prompt = f'{user_prompt}<|image_1|>{instruction_prompt} \n Query: {query}{prompt_suffix}{assistant_prompt}{instruction_prompt}'
                        else:
                            
                            prompt = f'{user_prompt}<|image_1|><|image_2|>{instruction_prompt} \n Query: {query}{prompt_suffix}{assistant_prompt}{instruction_prompt}'
                        image_paths= sample_path

                    else:
                        frame_paths, temp_dir = extract_sampled_frames(sample_path[0], num_samples=4)
                        prompt = f'{user_prompt}<|image_1|><|image_2|><|image_3|>{instruction_prompt} \n Query: {query}{prompt_suffix}{assistant_prompt}'
                        image_paths = frame_paths[:3]

            
                # Download and open image
                    images = [Image.open(im) for im in image_paths]
                    assert len(images) == prompt.count("<|image_"), "Mismatch between prompt tokens and images"
